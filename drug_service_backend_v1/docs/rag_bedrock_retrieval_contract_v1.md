...
이번 문서는 계약 설계 문서이며, 2026-05-14 기준 mock endpoint까지 구현했다. 남은 다음 단계는 아래와 같다.

```text
1. 프론트에서 /api/explanation-context 호출 연결
2. 프론트/챗봇 레이어에서 Bedrock prompt 구성
3. LLM 응답에 source_file/source_type/risk_sources 표시
4. 필요 시 evidence filtering 옵션 추가
```
