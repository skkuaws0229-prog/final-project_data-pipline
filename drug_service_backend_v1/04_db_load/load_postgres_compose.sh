...
psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -v ON_ERROR_STOP=1 -f /schema/postgres_schema.sql
...
\copy candidate_pool(candidate_id, disease_id, drug_id, canonical_drug_id, drug_name, rank, tier, score, target, target_pathway, evidence_summary, canonical_smiles, source_file, source_row_number, raw_json, is_final_candidate) FROM '/normalized/candidate_pool.csv' WITH (FORMAT csv, HEADER true)
...
UNION ALL
SELECT 'candidate_pool', COUNT(*) FROM candidate_pool
UNION ALL
