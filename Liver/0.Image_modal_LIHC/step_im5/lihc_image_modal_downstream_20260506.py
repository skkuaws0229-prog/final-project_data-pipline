...
CBIO_STUDY_ID = "lihc_tcga_pan_can_atlas_2018"
...
ROOT = package_root()
OUT_ROOT = ROOT / "local_data" / "lihc_image_modal_20260501_v1" / "0.Image_modal_LIHC"
...
def download_cbio_clinical(raw_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for level in ["PATIENT", "SAMPLE"]:
        path = raw_dir / f"cbio_lihc_{level.lower()}_clinical_long_{DATE_TAG}.json"
        if path.exists():
...
def download_cbio_mutations(raw_dir: Path) -> pd.DataFrame:
    path = raw_dir / f"cbio_lihc_driver_mutations_{DATE_TAG}.json"
    if path.exists():
        payload = json.loads(path.read_text(encoding="utf-8"))
    else:
        session = requests.Session()
...
def infer_subtype(row: pd.Series) -> str:
    ...
    if subtype and subtype.lower() not in {"nan", "none", "not reported", "lihc", "hepatocellular carcinoma"}:
        return subtype
    if bool(row.get("CTNNB1_mut", False)) or bool(row.get("AXIN1_mut", False)):
        return "WNT_CTNNB1_proxy"
    if bool(row.get("TP53_mut", False)):
        return "Proliferation_TP53_proxy"
    if bool(row.get("TERT_mut", False)) or bool(row.get("ALB_mut", False)):
        return "Hepatocyte_Differentiation_proxy"
    if bool(row.get("TP53_mut", False)):
        return "Proliferation_TP53_proxy"
    return "Unassigned"
...
def plot_pca(clusters: pd.DataFrame, out_path: Path, best_k: int, best_sil: float) -> None:
    ...
    plt.title(f"TCGA-LIHC WSI Embedding PCA Clusters (K={best_k}, silhouette={best_sil:.3f})")
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.tight_layout()
...
def run_survival(analysis: pd.DataFrame, out_dir: Path) -> dict[str, float]:
    ...
    plt.title(f"TCGA-LIHC Overall Survival by WSI Cluster (log-rank p={result.p_value:.3g})")
    plt.xlabel("Months")
...
def classify_tier(row: pd.Series) -> tuple[str, str]:
    ...
    if existing == "Tier1" or str(row.get("clinical_tier", "")).strip() == "1" or str(row.get("is_hcc_approved_anchor", "")).lower() == "true":
        return "Tier1", "Current/reference HCC therapy; validation control."
    if existing == "Tier2" or (not math.isnan(float(ct_count)) and float(ct_count) > 0):
        return "Tier2", "Evidence of liver cancer/HCC clinical investigation, but not current standard-control tier."
    ...
    if existing == "Tier4":
        return "Tier4", "Excluded by prior LIHC tiering as weak/non-actionable for this protocol."
    return "Tier3", "Named repurposing candidate without direct current-HCC-treatment status."


def cluster_profile(row: pd.Series) -> tuple[str, str, list[str]]:
    muts = {g: float(row.get(f"{g}_mut_rate", 0.0)) for g in DRIVER_GENES}
    if max(muts["CTNNB1"], muts["AXIN1"]) >= max(muts["TP53"], muts["TERT"], muts["ARID1A"], muts["ALB"]):
        return "WNT/CTNNB1-like WSI Cluster", "WNT-beta-catenin and hepatocyte differentiation biology", ["CTNNB1", "AXIN1", "WNT", "BETA-CATENIN"]
    if muts["TP53"] >= max(muts["CTNNB1"], muts["TERT"], muts["ARID1A"], muts["ALB"]):
        return "TP53-Proliferative-like WSI Cluster", "TP53/cell-cycle/proliferative injury biology", ["TP53", "CDK", "DNA", "TOP", "MITOSIS", "MICROTUBULE"]
    if max(muts["TERT"], muts["ARID1A"], muts["ALB"]) > 0:
        return "TERT/Chromatin-Hepatocyte-like WSI Cluster", "TERT promoter, chromatin remodeling, and hepatocyte lineage biology", ["TERT", "ARID1A", "ALB", "CHROMATIN", "VEGF", "KINASE"]
    return "Mixed/Unassigned WSI Cluster", "Mixed LIHC morphology without a dominant driver proxy", ["VEGF", "KINASE", "CELL CYCLE"]
...
def drug_cluster_mapping(top30: pd.DataFrame, cluster_profiles: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in top30.iterrows():
        ...
        out = row.to_dict()
        out["lihc_4tier"] = tier
        out["lihc_4tier_rationale"] = reason
    ...
    for _, drug in tiered.iterrows():
        ...
        for _, profile in cluster_profiles.iterrows():
            ...
            if "Mitosis".upper() in target_text and "MITOSIS" in keywords:
                broad_match = True
            if "Other".upper() in target_text and str(drug.get("lihc_4tier")) in {"Tier1", "Tier2", "Tier3"}:
                broad_match = True
                ...
                mapping_rows.append(
                    {
                        ...
                        "lihc_4tier": drug.get("lihc_4tier"),
                        "match_keywords": "; ".join(matches) if matches else "broad_lihc_tier_context",
                        "hypothesis": (
                            f"{drug.get('drug_name_display', drug.get('DRUG_NAME'))} is a {drug.get('lihc_4tier')} "
...
def main() -> None:
    dirs = ensure_dirs()
    embedding_root = ROOT / "local_data" / "lihc_image_modal_20260501_v1" / "output" / "embeddings_mid"
    top30_path = ROOT / "local_data" / "lihc_image_modal_20260501_v1" / "input" / "lihc_top30_tiered_candidates.csv"

    slide_df, emb = read_embeddings(embedding_root)
    np.save(dirs["step_im2"] / "all_slide_embeddings_lihc_merged.npy", emb)
    slide_df.to_csv(dirs["step_im2"] / "all_slide_embeddings_lihc_merged_metadata.csv", index=False)
    qc = {
        "requested_shape_note": "LIHC Step3 parquet contains the available successfully embedded slide rows.",
    ...
    patient_clin.to_csv(dirs["raw"] / "cbio_lihc_patient_clinical_wide.csv", index=False)
    sample_clin.to_csv(dirs["raw"] / "cbio_lihc_sample_clinical_wide.csv", index=False)
    mutations.to_csv(dirs["raw"] / "cbio_lihc_driver_mutations.csv", index=False)
    ...
    analysis.to_csv(dirs["step_im4a"] / "lihc_cluster_clinical_mutation_table.csv", index=False)
    ...
    cluster_profiles.to_csv(dirs["step_im4b"] / "lihc_cluster_pathway_profiles.csv", index=False)

    top30 = pd.read_csv(top30_path)
    tiered, mapping = drug_cluster_mapping(top30, cluster_profiles)
    tiered.to_csv(dirs["step_im4c"] / "lihc_top30_4tier_classification.csv", index=False)
    mapping.to_csv(dirs["step_im4c"] / "lihc_top30_cluster_drug_hypotheses.csv", index=False)
    tiered.to_csv(OUT_ROOT / "lihc_top30_4tier_classification.csv", index=False)
    mapping.to_csv(OUT_ROOT / "im4c_lihc_cluster_drug_mapping.csv", index=False)
    ...
    (OUT_ROOT / "step_im5_lihc_image_modal_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
    cleanup = {
        "raw_wsi_prefix": "s3://say2-4team/lihc_image_modal_20260501_v1/wsi_raw/",
        "delete_command": "aws s3 rm s3://say2-4team/lihc_image_modal_20260501_v1/wsi_raw/ --recursive",
        "status": "not_executed",
    ...
    zip_path = dirs["step_im5"] / "lihc_image_modal_downstream_results.zip"
    if zip_path.exists():
        zip_path.unlink()
    ...
    (OUT_ROOT / "lihc_image_modal_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
